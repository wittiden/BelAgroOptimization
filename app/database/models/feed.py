...
if TYPE_CHECKING:
    ...
    from app.database.models.crop import CropModel


class FeedDataModel(Base):
    ...
    scenario: Mapped["ScenarioModel"] = relationship(back_populates="feed_data")

    def __repr__(self) -> str:
        return f"<FeedData(type={self.feed_type}, price={self.price_byn_per_centner})>"


class FeedOutputModel(Base):
    ...
    scenario: Mapped["ScenarioModel"] = relationship(back_populates="feed_outputs")
    crop: Mapped["CropModel"] = relationship(back_populates="feed_outputs")

    def __repr__(self) -> str:
        return f"<FeedOutput(crop={self.crop_id}, feed={self.feed_type}, ratio={self.output_ratio})>"


class FeedAllocationModel(Base):
    ...
    result: Mapped["OptimizationResultModel"] = relationship(back_populates="feed_allocations")
